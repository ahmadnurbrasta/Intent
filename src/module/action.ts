...
export async function actions(
    ...
    for (let i = 0; i < json_data.length; i++) {
        ...
        console.log(chalk.cyan(`[INTENT NAME] ${element['intent_name']}`));

        let question_count_per_topic = 0;

        for (const [key, value] of Object.entries(element)) {
            if (key.startsWith("sample_text") && value !== "" && value !== null) {
                question_count++;
                ...
                await env.wait_reply(page, question, 20);
                ...
                let respond_csv = String(element["respond"]).trim();
                respond_csv = envstatus.respond_csv_correction(respond_csv);

                
                const diff_strings = envstatus.diff_strings(respond_bot, respond_csv)
                const scoreData = await envstatus.probability(respond_bot, respond_csv);
                const status = envstatus.status(scoreData);
                ...
                console.log(chalk.magenta(`  ${status_icon} | Score: ${scoreData.toFixed(2)} [${status}] | Durasi Pertanyaan: ${dur_q}s | Bubble: ${total_bubbles}`));
                // console.log(chalk.white(`  🟡 | Explanation: ${scoreData.explanation}`));

                const data_bot = {
                    no: key, parent_id: element["parent_id"], intent_name: element["intent_name"], sample_text: question,
                    response_kb: respond_csv, response_llm: respond_bot, status: status,
                    probability: scoreData, diff_strings: diff_strings, duration: dur_q, image_capture: image_capture
                };
                ...
                const data_summary = {
                    id_test: id_test, tester_name: tester_name,
        ...
        if (intent_count < json_data.length && intent_count % 1 === 0) {
            ...
            console.log(`[RESTART] Intent ke-${intent_count} selesai → restart browser...\n`);
            ...
            await env.prechat_form(page, "Halo", tester_name, tester_email, tester_phone);
